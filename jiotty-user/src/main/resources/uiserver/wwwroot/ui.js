$(function () {
    ...
    function optionToHtml(opt) {
        ...
        if (type === 'text') {
            ...
            return '<fieldset><legend>' + label + '</legend>' +
                ...
                '</div>' +
                '</fieldset>';
        } else if (type === 'car_integrations') {
            const configs = (opt.value && opt.value.configsByIntegrationId) || {};
            let html = '<fieldset class="mb-3"><legend>' + label + '</legend>';
            Object.keys(configs).forEach(function (integrationId) {
                const config = configs[integrationId];
                html += '<div class="card mb-2 p-2"><strong>' + escapeHtml(integrationId) + '</strong><dl class="mb-0 ms-2">';
                Object.keys(config).forEach(function (attr) {
                    if (attr === 'integrationId') return;
                    html += '<dt>' + escapeHtml(attr) + '</dt><dd>' + escapeHtml(String(config[attr])) + '</dd>';
                });
                html += '</dl></div>';
            });
            html += '</fieldset>';
            return html;
        } else if (type === 'energy_provider_integration') {
            let html = '<fieldset class="mb-3"><legend>' + label + '</legend>';
            if (opt.value) {
                html += '<div class="card mb-2 p-2"><strong>' + escapeHtml(opt.value.integrationId || '') + '</strong><dl class="mb-0 ms-2">';
                Object.keys(opt.value).forEach(function (attr) {
                    if (attr === 'integrationId') return;
                    html += '<dt>' + escapeHtml(attr) + '</dt><dd>' + escapeHtml(String(opt.value[attr])) + '</dd>';
                });
                html += '</dl></div>';
            } else {
                html += '<em>Not configured</em>';
            }
            html += '</fieldset>';
            return html;
    ...
    function buildOptionsTabs(tabs) {
        ...
        tabs.forEach(function (tab) {
            ...
            const sorted = (tab.options || []).slice().sort(function (a, b) {
                return (a.order || 0) - (b.order || 0);
            });
            const inner = sorted.map(optionToHtml).join('');
            content.append(
    ...
    function optionsStructureChanged(oldTabs, newTabs) {
        if (oldTabs.length !== newTabs.length) return true;
        for (let i = 0; i < oldTabs.length; i++) {
            if (oldTabs[i].id !== newTabs[i].id || oldTabs[i].name !== newTabs[i].name) return true;
            const oldOpts = (oldTabs[i].options || []).slice().sort(orderComparator);
            const newOpts = (newTabs[i].options || []).slice().sort(orderComparator);
            if (oldOpts.length !== newOpts.length) return true;
            for (let j = 0; j < oldOpts.length; j++) {
                const o = oldOpts[j], n = newOpts[j];
                if (o.key !== n.key || o.type !== n.type || o.label !== n.label || o.order !== n.order) return true;
                if (o.type === 'select' && JSON.stringify(o.options) !== JSON.stringify(n.options)) return true;
                if (o.type === 'multiselect' &&
                    JSON.stringify(Object.keys(o.allOptions || {})) !== JSON.stringify(Object.keys(n.allOptions || {}))) return true;
            }
        }
        return false;
    }
    ...
    function currentDomValue($el, type) {
        if (type === 'checkbox') return $el.is(':checked');
        if (type === 'multiselect') return ($el.val() || []).slice().sort().join(',');
        return $el.val() || '';
    }

    function patchOptionValues(newTabs) {
        newTabs.forEach(function (tab) {
            (tab.options || []).forEach(function (opt) {
                const key = opt.key;
                const type = opt.type;
                if (type === 'car_integrations' || type === 'energy_provider_integration') {
                    // read-only display types — re-render their container
                    const $wrapper = $('[data-option="' + key + '"]').closest('fieldset');
                    if ($wrapper.length) $wrapper.replaceWith(optionToHtml(opt));
                    return;
                }
                const $el = $('[data-option="' + key + '"]');
                if ($el.length === 0) return;
                if (type === 'chat') {
                    const $hist = $el.find('[data-history-for="' + key + '"]');
                    const newVal = opt.historyText || '';
                    if ($hist.length && $hist.val() !== newVal) {
                        $hist.val(newVal);
                ...
                // value differs — patch DOM and update tracked lastVal
                if (type === 'checkbox') {
                    $el.prop('checked', newDtoVal);
                    $el.data('lastVal', newDtoVal);
                } else if (type === 'multiselect') {
                    const arr = opt.selectedIds || [];
                    $el.val(arr);
                    $el.data('lastVal', arr.join(','));
                } else {
                    const val = (type === 'duration') ? (opt.valueHuman != null ? opt.valueHuman : '') : (opt.value != null ? opt.value : '');
                    $el.val(val);
                    $el.data('lastVal', val);
                }
            });
    ...
    function attachOptionHandlers() {
        $('[data-option][data-option-type!="chat"][data-option-type!="location"]').each(function () {
            ...
            // Track last saved value on the element so patchOptionValues can update it
            $el.data('lastVal', getCurrentValue());
            ...
            function maybePost() {
                ...
                $.post('api/options', {name, value: curr})
                    .done(function (resp) {
                        ...
                        if (type === 'checkbox') {
                            const boolVal = resp === true;
                            $el.prop('checked', boolVal);
                            $el.data('lastVal', boolVal);
                        } else if (type === 'multiselect') {
                            const arr = resp || [];
                            $el.val(arr);
                            $el.data('lastVal', arr.join(','));
    ...
    function subscribeOptions() {
        ...
        DisplayableSse.onOptionsUpdate(function (data) {
            ...
            if (!lastOptionsSnapshot || optionsStructureChanged(lastOptionsSnapshot, data.tabs)) {
                ...
                patchOptionValues(data.tabs);
            }
            lastOptionsSnapshot = data.tabs;
        });
    }
